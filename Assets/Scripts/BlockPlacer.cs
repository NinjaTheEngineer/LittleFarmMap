...
public class BlockPlacer : NinjaMonoBehaviour {
    ...
    private void Update() {
        ...
        if (IsPlacingBlock && Physics.Raycast(cam.position, cam.forward * rayDistance, out raycastHit, 10, layersToHit)) {
            HandlePreviewPosition();
            if (Input.GetMouseButtonDown(0) && previewBlock.CanBePlaced) {
                PlaceBlock();
            }
        }
    }
